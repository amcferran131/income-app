  ...
  try {
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages:[{role:"user",content:"What is the current dividend per share per payment for " + ticker + "? Also what is the payment frequency and security type? Reply ONLY with raw JSON, no markdown: {"name":"full name","type":"stock|reit|bond|preferred|cd","divPerShare":0.00,"freqId":"monthly|q_jan|q_feb|q_mar|semi_jan|semi_feb|annual_dec|annual_jun","notes":"brief"} or {"error":"not found"}"}]
      })
    });
  ...
  return (
    <div className="welcome">
      ...
      <div className="wsteps">
        <div className="wstep wstep-hi">
          <div className="wnum">1</div>
          <div><strong>Try the demo first</strong><br/><span>See how everything works with a sample portfolio</span></div>
        </div>
        <div className="wstep">
          <div className="wnum">2</div>
          <div><strong>Import your holdings</strong><br/><span>Upload a CSV export from Schwab, Fidelity, Vanguard, or Robinhood</span></div>
        </div>
        <div className="wstep">
          <div className="wnum">3</div>
          <div><strong>AI fills dividend rates</strong><br/><span>One click looks up current rates for every position</span></div>
        </div>
        <div className="wstep">
          <div className="wnum">4</div>
          <div><strong>See your income forecast</strong><br/><span>Charts, calendar, and monthly projections instantly</span></div>
        </div>
      </div>
      <div className="wbtns">
        <button className="wbtn-demo" onClick={onDemo}>Step 1 - Load Demo Portfolio</button>
        <button className="wbtn-import" onClick={()=>fileRef.current&&fileRef.current.click()}>Step 2 - Import Brokerage CSV</button>
                  ...
                  {holdings.map(h=>{
                    ...
                    return (
                      <tr key={h.id} style={{background:flag?"#fffbeb":""}}>
                        ...
                        <td style={{fontFamily:"monospace",fontSize:10,color:an>0?"#10b981":"#cbd5e1"}}>{an>0?fmt(an):"--"}</td>
